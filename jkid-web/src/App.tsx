...
function Landing() {
  return <Box height={"100%"} display={"flex"} alignItems={"center"}><Grid container sx={{flexGrow: 1}}>
    ...
    <Grid xs={10} md={6} sx={{display: "flex", flexDirection: "column", gap: 2}}>
      ...
      <Typography>原则上，我们只允许清华大学物理系与致理书院物理方向的同学注册。其他院系的同学如果有注册意愿，请通过邮件单独联系我们。</Typography>
      <Typography>清华大学在校学生请点击下面的“在校学生注册”进入注册流程。我们将会先将你重定向到 Tsinghua Git，请在那里先登录，登录完毕后，你将回到注册页面。我们需要获取你的<b>真实姓名、学号、学生邮箱</b>以便审核你的身份。除此之外，不会读取任何其他信息。</Typography>
      <Typography>如果你不再拥有清华大学电子身份（例如，已经从清华大学毕业），不用担心！请点击<Link>其他人员注册</Link>以进入注册页面。</Typography>
      <Button size={"large"} variant={"contained"} sx={{ alignSelf: "center" }} href={
        new URL("/register/fire", import.meta.env.VITE_BACKEND_ADDRESS).toString()
      }>在校学生注册</Button>
